...
def final_fix():
    ...
    # 生成内容区域和网站数据
    content_html, sites_data_json = generate_content(data['groups'], category_map)
...
def get_favicon_url(url):
    """获取网站favicon URL，使用Google favicon服务"""
    if not url or url == '#':
...
def generate_content(groups, category_map):
    ...
    for group in groups:
        ...
        for i, site in enumerate(group_sites):
            ...
            site_icon = get_favicon_url(site_url)
