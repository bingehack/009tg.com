        ...
        <div class="main-content">
            <nav class="navbar user-info-navbar" role="navigation">
                ...
                <ul class="user-info-menu left-links list-inline list-unstyled">
                    ...
                    {lang_switcher}
                    <li class="hidden-sm hidden-xs" style="margin-left: 10px; display: flex; align-items: center;">
                        <div class="search-box-wrapper" style="position: relative; display: flex; align-items: center;">
                            <input type="text" id="site-search-input" class="form-control" 
                                   placeholder="{search_placeholder}" 
                                   style="width: 200px; height: 32px; font-size: 13px; border-radius: 16px; padding-left: 32px; margin: 0;"
                                   autocomplete="off">
                            <i class="fa-search" style="position: absolute; left: 12px; top: 50%; transform: translateY(-50%); color: #999; font-size: 13px;"></i>
    ...
    <script>
        ...
        function performSearch(query) {{
            ...
            for (var i = 0; i < allSitesFlat.length; i++) {{
                ...
                var kwMatch = site.keywords.toLowerCase().indexOf(queryLower) !== -1;

                if (nameMatch || descMatch || urlMatch || catMatch || kwMatch) {{
                    // 计算匹配分数，名称匹配权重最高
                    var score = 0;
